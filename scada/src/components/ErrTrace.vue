        ...
        <v-flex>
            <v-data-table
                ...
                <template v-slot:top>
                    ...
                    flat
                    >
                <v-toolbar-title>Registro de Errores</v-toolbar-title>
                        <v-divider
                            class="mx-4"
                            inset
                            vertical
    ...
    export default {
        data() {
            return  {
                    ...
                    headers: [
                        ...
                        { text: 'Tabla Proceso', value: 'procedurName' },
                        { text: 'Código', value: 'errNumber' },
                        { text: 'Origen', value: 'errSource' },
                        { text: 'Descripción', value: 'errDescription' }
                    ],
                    search: '',
        ...
        methods: {
            listar(){
                ...
                axios.get('api/ErrTraces/Listar',configuracion).then(function(response){
                    //console.log(response)
                    me.errtraces=response.data;
                }).catch(function(error){
                    console.log(error)
                    errTraceguardarAxios("ErrTrace - Listar", error);
                });        
            },
            selectbuscar(){
                let me=this;
                ...
                me.resultBusqueda=[];
                axios.get('api/ErrTraces/ListarFiltro/'+this.palabraClave,configuracion).then(function(response){
                        resultBusquedaArray=response.data;
                        resultBusquedaArray.map(function(x){
                        me.resultBusqueda.push({dateAndTime: x.dateAndTime, procedurName: x.procedurName, errNumber: x.errNumber, errSource: x.errSource, errDescription: x.errDescription})
                    });
                    }).catch(function(error){
                        errTraceguardarAxios("ErrTrace - ListarFiltro", error);
                    console.log(error)
                });
            },
            selectbuscarfecha(){
                ...
                me.resultBusqueda=[];
                //console.log(today);
                //console.log('StartTime: ' + (this.getdatetime(me.date1 + " " + me.time1)));
                //console.log('EndTime: ' + (this.getdatetime(me.date2 + " " + me.time2).toISOString()));
                axios.post('api/ErrTraces/ListarFiltroFecha',{
                        'StartTime': (this.getdatetime(me.date1 + " " + me.time1)).toISOString(),
                        'EndTime': (this.getdatetime(me.date2 + " " + me.time2)).toISOString()
                    ...
                    errTraceguardarAxios("ErrTrace - ListarFiltroFecha", error);
                    console.log(error)
                });
            },
            copiartabla(array){
